...
* `vv-input-range`, a slider for a numeric value between known bounds, with the value picked shown next to the track. Elements are `label`, `wrapper`, `input-before`, `input`, `input-after`, `value`, `unit` and `hint`, modifiers are `valid`, `invalid`, `loading` and `readonly`, and the `disabled` state also applies through `:has(input[disabled])`, so plain markup is styled without the modifier class. A range input takes no `readonly` attribute. The recommended markup disables the native control and adds `vv-input-range--readonly`, which undoes the disabled look: full opacity, the default cursor and no underline. `vv-select` resolves its own missing `readonly` the same way.

  The slider itself is the native control, drawn by the browser and colored by `accent-color`, so the filled part of the track follows the thumb with no script and the `valid` and `invalid` modifiers recolor it by inheritance. Around it the field is the chrome of the other ones: the same label, the same filled wrapper with the underline that grows on focus and animates under `loading`, the same hint. Turning the slider back on takes `appearance: auto` on the input, because the reset drops every user agent style with `all: unset`, the same reason it reverts the appearance of `meter`. `auto` and not `revert`: a CSS-wide keyword read back from a custom property leaves the property at its initial value, and the generator routes every declaration through one.

  What the browser draws stays the browser's. The thumb is round on Chrome and grey on Firefox, the empty part of the track keeps its light grey even under `color-scheme: dark`, because no engine adapts it, and a readonly field shows the greyed slider of a disabled control, which is what it is. A project that needs the track drawn instead can deep-merge the pseudo-element maps below, at the price of writing the filled share on the block from a script.
* `$input` gains a `range` sub-map, so a slider is themed together with the other fields: `--input-range-accent-color` sets the color of track and thumb for every range field of the project, and `accent-color` on a block recolors one of them.
...
* The BEM generator accepts the pseudo-elements of a range input: `-webkit-slider-runnable-track`, `-webkit-slider-thumb`, `-moz-range-track`, `-moz-range-progress` and `-moz-range-thumb`, so a component map can draw a slider of its own instead of the native one.
